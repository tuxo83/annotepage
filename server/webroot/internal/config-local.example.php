...
return array(
    ...
    // does not know is an error worth reporting there, most often a tag copied
    // from another site.
    //
    // On a relay it is what makes "no account" true: any well-formed id is
    // served, there is no registration and no origin lock -- there cannot be
    // one, since the id is in the tag of every annotated page. A stranger can
    // therefore write into a project whose id they have seen. What they write
    // is bytes: the server cannot read them, the client cannot decrypt them,
    // and the panel shows them counted and unreadable. It is a nuisance, not a
    // disclosure -- and a team that wants the lock declares its project here
    // with its origins, or runs its own server.
    //
    // Encrypted mode is not optional on a relay, whatever a project asks for;
    // that refusal comes before this key.
    ...
    // LETTING THIS SERVER INSTALL ITS OWN UPDATES, from a web request, with no
    // cron and no shell. Off, and the installer never writes it: it is the last
    // of the three ways to stay current, and the only one that costs a
    // permission.
    //
    // WHAT IT COSTS, and it does not go away when you set it back to false: the
    // code directory must be writable by the user PHP runs as. From that
    // moment any bug anywhere on this account that can write a file -- in this
    // code, in a neighbouring application, in a plugin nobody remembers
    // installing -- stops being a defacement and becomes permanent code
    // execution. The permission stays until somebody takes it away.
    //
    // It also needs a PHP interface that can answer the visitor BEFORE doing
    // more work: php-fpm and LiteSpeed can, cgi-fcgi cannot, and where it
    // cannot the key is read and declined on every write. ?action=diagnostic
    // says which one this host runs.
    // 'auto_update' => true,

    // HOW LONG A THREAD IS KEPT, counted from its LAST message, so a reply is
    // never cut off its remark and a live discussion is never cut short. The
    // whole thread goes at once, and nobody chooses which: there is no
    // moderation here and no takedown, which is the point of saying age and
    // only age.
    //
    // The installer writes 90 into every configuration it creates. 0 keeps
    // everything for ever: the client then announces no age, and this server
    // removes nothing. While it is set, the panel on the annotated pages says
    // so and every export carries it in its header -- nobody discovers it late.
    // 'max_note_age_days' => 90,

    // WHAT THE WHOLE SERVER HOLDS -- projects, notes, pages -- answered on the
    // call every annotated page already makes, next to that project's own
    // figures. Off, and no client draws it.
    //
    // Anyone who can open one annotated page can read this answer. On a relay
    // that means every visitor of every site using it learns how many teams
