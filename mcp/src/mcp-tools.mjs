...
const KEY_ARG = {
    ...
    description: 'The 43-character key of the project, when the project is not in '
        ...
        + 'that page and look in TWO places — a site declares its settings in '
        + 'either, and some write both:\n'
        + '   - the annotepage tag at the end of the document, carrying data-key '
        + '(this argument) and data-server (the "api" argument);\n'
        + '   - a window.annotepageConfig object declared before that tag, with '
        + 'the same settings under their bare names: key, server. A page whose '
        + 'scripts are concatenated, or served by a plugin, may carry only this '
        + 'one — finding no data-key attribute does NOT mean the key is private.\n'
        + 'This is the normal case, it needs nobody, and it is what makes a review '
...
const CARRIES_ITS_OWN_PROJECT =
    ...
    + '"api" and "key" read off the page itself — from the annotepage tag '
    + '(data-server, data-key) or from a window.annotepageConfig object declared '
    + 'before it (server, key), whichever that page carries — plus "origin", '
    + 'which is simply the origin of that '
    ...
    return [
...

        {
            ...
            description:
                ...
                + 'nothing.\n\n'
                + 'AND ASK HERE, NEVER IN THE OPERATOR\'S TERMINAL. If a remark is '
                + 'ambiguous, the person who can settle it is the one who wrote it, '
                + 'and they are reading this thread -- not the console you are '
                + 'running in. A question asked there is asked of somebody who was '
                + 'not there.\n\n'
                + 'Better still, ask nothing: state the reading you acted on. "I read '
                + 'this as the label wrapping below 380px, and fixed that" costs the '
                + 'reviewer nothing when you were right, which is most of the time, '
                + 'and one button when you were not -- reopening exists for exactly '
                + 'that. Nothing here notifies anybody: a question left in a thread '
                + 'waits until somebody happens to open the page again.\n\n'
                + 'KEEP IT SHORT. A few sentences, and prefer the MEASUREMENT to the '
                + 'account of the work: "the label holds one line down to 317px" is '
                + 'the answer; how it was found is not. The reviewer is reading this '
                + 'in a panel beside the page they were looking at, not in a report, '
                + 'and a long reply buries the number they came for. If something '
                + 'really needs explaining at length, say the short version here and '
                + 'put the rest where long things belong -- a commit message, a '
                + 'comment in the code.\n\n'
